...
static bool MacroBodyEndsInBackslash(llvm::StringRef MacroBody) {
...
static void AddImplicitIncludeMacros(MacroBuilder &Builder, llvm::StringRef File) {
...
static void AddImplicitIncludePCH(MacroBuilder &Builder, Preprocessor &PP,
                                  ...
                                  llvm::StringRef ImplicitIncludePCH) {
...
static void DefineTypeSize(const llvm::Twine &MacroName, unsigned TypeWidth,
...
static void DefineTypeSize(const llvm::Twine &MacroName, TargetInfo::IntType Ty,
                           const TargetInfo &TI, MacroBuilder &Builder) {
  DefineTypeSize(MacroName, TI.getTypeWidth(Ty), TI.getTypeConstantSuffix(Ty),
                 TI.isTypeSigned(Ty), Builder);
}

static void DefineFmt(const LangOptions &LangOpts, const llvm::Twine &Prefix,
  ...
  llvm::for_each(llvm::StringRef(IsSigned ? "di" : "ouxX"), Emitter);
  ...
  if (LangOpts.C23 && !IsSigned)
    llvm::for_each(llvm::StringRef("bB"), Emitter);
}
...
static void DefineTypeWidth(const llvm::Twine &MacroName, TargetInfo::IntType Ty,
                            const TargetInfo &TI, MacroBuilder &Builder) {
...
static void DefineTypeSizeAndWidth(const llvm::Twine &Prefix, TargetInfo::IntType Ty,
                                   const TargetInfo &TI,
                                   MacroBuilder &Builder) {
...
static void DefineExactWidthIntType(const LangOptions &LangOpts,
  ...
  DefineFmt(LangOpts, Prefix + llvm::Twine(TypeWidth), Ty, TI, Builder);

  llvm::StringRef ConstSuffix(TI.getTypeConstantSuffix(Ty));
  Builder.defineMacro(Prefix + llvm::Twine(TypeWidth) + "_C_SUFFIX__", ConstSuffix);
}
...
static void InitializeStandardPredefinedMacros(const TargetInfo &TI,
  ...
  if (LangOpts.HLSL) {
    ...
    Builder.defineMacro("__HLSL_VERSION",
                        llvm::Twine((unsigned)LangOpts.getHLSLVersion()));
    ...
    Builder.defineMacro("__SHADER_STAGE_VERTEX",
                        llvm::Twine((uint32_t)ShaderStage::Vertex));
    Builder.defineMacro("__SHADER_STAGE_PIXEL",
                        llvm::Twine((uint32_t)ShaderStage::Pixel));
    Builder.defineMacro("__SHADER_STAGE_GEOMETRY",
                        llvm::Twine((uint32_t)ShaderStage::Geometry));
    Builder.defineMacro("__SHADER_STAGE_HULL",
                        llvm::Twine((uint32_t)ShaderStage::Hull));
    Builder.defineMacro("__SHADER_STAGE_DOMAIN",
                        llvm::Twine((uint32_t)ShaderStage::Domain));
    Builder.defineMacro("__SHADER_STAGE_COMPUTE",
                        llvm::Twine((uint32_t)ShaderStage::Compute));
    Builder.defineMacro("__SHADER_STAGE_AMPLIFICATION",
                        llvm::Twine((uint32_t)ShaderStage::Amplification));
    Builder.defineMacro("__SHADER_STAGE_MESH",
                        llvm::Twine((uint32_t)ShaderStage::Mesh));
    Builder.defineMacro("__SHADER_STAGE_LIBRARY",
                        llvm::Twine((uint32_t)ShaderStage::Library));
    ...
    Builder.defineMacro("__SHADER_TARGET_STAGE", llvm::Twine(StageInteger));
    // Add target versions
    if (TI.getTriple().getOS() == llvm::Triple::ShaderModel) {
      llvm::VersionTuple Version = TI.getTriple().getOSVersion();
      Builder.defineMacro("__SHADER_TARGET_MAJOR", llvm::Twine(Version.getMajor()));
      unsigned Minor = Version.getMinor().value_or(0);
      Builder.defineMacro("__SHADER_TARGET_MINOR", llvm::Twine(Minor));
    }
    return;
  }
  // C++ [cpp.predefined]p1:
  ...
  if (!LangOpts.CPlusPlus) {
    ...
    Builder.defineMacro("__STDCPP_DEFAULT_NEW_ALIGNMENT__",
                        llvm::Twine(TI.getNewAlign() / TI.getCharWidth()) +
...
void DefineFixedPointMacros(const TargetInfo &TI, MacroBuilder &Builder,
  ...
  Builder.defineMacro(MacroPrefix + "_FBIT__", llvm::Twine(Scale));
  Builder.defineMacro(
      MacroPrefix + "_MAX__",
...
#undef TOSTR2
  ...
  if (LangOpts.GNUCVersion != 0) {
    ...
    if (LangOpts.CPlusPlus) {
      Builder.defineMacro("__GNUG__", llvm::Twine(GNUCMajor));
  ...
  Builder.defineMacro("__OBJC_BOOL_IS_BOOL",
                      llvm::Twine(TI.useSignedCharForObjCBool() ? "0" : "1"));
  ...
  if (LangOpts.FixedPoint) {
    ...
    Builder.defineMacro("__SACCUM_IBIT__", llvm::Twine(TI.getShortAccumIBits()));
    Builder.defineMacro("__USACCUM_IBIT__",
                        llvm::Twine(TI.getUnsignedShortAccumIBits()));
    Builder.defineMacro("__ACCUM_IBIT__", llvm::Twine(TI.getAccumIBits()));
    Builder.defineMacro("__UACCUM_IBIT__", llvm::Twine(TI.getUnsignedAccumIBits()));
    Builder.defineMacro("__LACCUM_IBIT__", llvm::Twine(TI.getLongAccumIBits()));
    Builder.defineMacro("__ULACCUM_IBIT__",
                        llvm::Twine(TI.getUnsignedLongAccumIBits()));
  }
  ...
  Builder.defineMacro("__GCC_DESTRUCTIVE_SIZE", llvm::Twine(Destructive));
  Builder.defineMacro("__GCC_CONSTRUCTIVE_SIZE", llvm::Twine(Constructive));
...
#undef DEFINE_LOCK_FREE_MACRO
  ...
  if (unsigned PICLevel = LangOpts.PICLevel) {
    ...
    Builder.defineMacro("__pic__", llvm::Twine(PICLevel));
    if (LangOpts.PIE) {
      Builder.defineMacro("__PIE__", llvm::Twine(PICLevel));
      Builder.defineMacro("__pie__", llvm::Twine(PICLevel));
    }
  }
